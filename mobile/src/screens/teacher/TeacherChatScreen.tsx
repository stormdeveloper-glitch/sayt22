...
export const TeacherChatScreen: React.FC = () => {
  ...
  const getChatRooms = (): ChatRoom[] => {
    const rooms: ChatRoom[] = [
      ...
      {
        id: 'all_teachers',
        name: 'O\'qituvchilar guruhi',
        toType: 'all_teachers',
        subtitle: 'O\'qituvchilar o\'rtasida chat',
        icon: 'people-outline',
      },
    ];

    // Add students as direct message targets
    if (db && db.students) {
  ...
  const getFilteredMessages = () => {
    ...
    return db.messages.filter((m) => {
      ...
      // 2. All teachers group chat
      if (activeRoom.toType === 'all_teachers' && m.toType === 'all_teachers') return true;

      // 3. Direct messaging to specific student
      if (activeRoom.toType === 'specific_student') {
        const isFromMeToStudent =
          m.fromType === 'teacher' &&
          m.fromId === currentUser.id &&
          m.toType === 'specific_student' &&
          m.toId === activeRoom.id;
        const isFromStudentToMe =
          m.fromType === 'student' &&
          m.fromId === activeRoom.id &&
          m.toType === 'specific_teacher' &&
          m.toId === currentUser.id;
        return isFromMeToStudent || isFromStudentToMe;
      }
