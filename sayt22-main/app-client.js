...
function isSuperAdmin() { if (curType !== 'admin') return false; const a = getAdminById(curAdminId); return !!(a && a.role === 'super' && a.status === 'active'); }
...
// ========== RATE LIMITING SYSTEM (Prevents API limit hits) ==========
const _saveState = {
  debounceTimer: null,
  lastBackendSync: 0,
  minIntervalMs: 3000,           // Minimum 3 seconds between backend requests
  debounceDelayMs: 2000,         // Delay batching by 2 seconds
  backendWaitMs: 2000,           // Initial backend wait (2s)
  isBackendBusy: false,          // Only 1 backend request at a time
  retryCount: 0,
  maxRetryWait: 60000            // Max wait time 60 seconds
};

function _scheduleBackendSync() {
  if (_saveState.debounceTimer) clearTimeout(_saveState.debounceTimer);
  
  _saveState.debounceTimer = setTimeout(async () => {
    // Calculate wait time considering last sync and min interval
    const timeSinceLastSync = Date.now() - _saveState.lastBackendSync;
    const needWait = Math.max(0, _saveState.minIntervalMs - timeSinceLastSync);
    
    if (needWait > 0) {
      // Schedule another attempt after the required wait
      _saveState.debounceTimer = setTimeout(_scheduleBackendSync, needWait);
      return;
    }
    
    // Backend is busy, reschedule
    if (_saveState.isBackendBusy) {
      _saveState.debounceTimer = setTimeout(_scheduleBackendSync, 500);
      return;
    }
    
    _saveState.isBackendBusy = true;
    try {
      ...
      _saveState.lastBackendSync = Date.now();
      
      if (res.status === 429) {
        // Rate limited - increase wait time exponentially
        _saveState.backendWaitMs = Math.min(_saveState.backendWaitMs * 2, _saveState.maxRetryWait);
        _saveState.minIntervalMs = _saveState.backendWaitMs;
        console.warn(`[Rate Limit] 429 - Next attempt in ${_saveState.backendWaitMs}ms`);
        toast(`Server meşghul. ${_saveState.backendWaitMs / 1000}s kuting.`, 'warn');
      } else if (!res.ok) {
        console.warn(`[Backend] Error ${res.status}`, res);
      } else {
        // Success - reset wait time
        _saveState.backendWaitMs = 2000;
        _saveState.minIntervalMs = 3000;
        _saveState.retryCount = 0;
      }
    } catch (e) {
      console.warn('[Save Error]', e);
    } finally {
      _saveState.isBackendBusy = false;
    }
  }, _saveState.debounceDelayMs);
}

// ========== COMMAND RATE LIMITING ==========
const _cmdRateLimit = {
  texno: { lastTime: 0, cooldownMs: 4000 }  // /texno once every 4 seconds
};

function _canExecuteCommand(cmdName, cooldownMs = 4000) {
  const state = _cmdRateLimit[cmdName] || { lastTime: 0, cooldownMs };
  const now = Date.now();
  const timeSinceLastExec = now - state.lastTime;
  
  if (timeSinceLastExec < state.cooldownMs) {
    const waitMs = state.cooldownMs - timeSinceLastExec;
    const waitSec = Math.ceil(waitMs / 1000);
    toast(`Kutish uchun ${waitSec}s qoldi.`, 'warn');
    return false;
  }
  
  state.lastTime = now;
  _cmdRateLimit[cmdName] = state;
  return true;
}

async function save() {
  // Always save to localStorage immediately (no data loss)
  const d = { seedVersion, students, transactions: txs, nextStudentId: nid, teachers, nextTeacherId: ntid, admins, nextAdminId: naid, adminRequests, nextRequestId: nrid, messages, groups, tests, plans };
  try { localStorage.setItem(SK, JSON.stringify(d)); } catch (e) { console.warn('LocalStorage save error', e); }
  
  // Schedule backend sync (batched, rate-limited)
...
function showPage(pid, el) {
  ...
  if (pid === 'dpg') renderSD();
  if (pid === 'rpg') { if (!_rankingMeta || !document.getElementById('rpgIdent')?.options?.length) onRpgScopeChange(); else renderFLb(); }
...
let _rankingMeta = null;
async function ensureRankingMeta() {
  if (_rankingMeta) return _rankingMeta;
  try {
    const res = await fetch('/api/ranking/meta');
...
function viewerParams() {
  if (curType === 'student') return { viewerRole: 'student', viewerId: curId };
  if (curType === 'teacher') return { viewerRole: 'teacher', viewerId: curTeacherId };
  if (curType === 'admin') return { viewerRole: 'admin', viewerId: curAdminId };
  return {};
}
function onRpgScopeChange() {
  const scope = document.getElementById('rpgScope')?.value || 'overall';
  const identWrap = document.getElementById('rpgIdentWrap');
  const identLabel = document.getElementById('rpgIdentLabel');
  const identSel = document.getElementById('rpgIdent');
  const subEl = document.getElementById('rpgSubtitle');
  const s = getStudentById(curId);
  if (!identWrap || !identSel) return;
  (async () => {
    const meta = await ensureRankingMeta() || { subjects: [], groups: [], teachers: [] };
    if (scope === 'overall') {
      identWrap.style.display = 'none';
      if (subEl) subEl.textContent = '// Barcha talabalar umumiy reytingi';
    } else if (scope === 'subject') {
      identWrap.style.display = '';
      identLabel.textContent = 'FAN';
      identSel.innerHTML = meta.subjects?.map(x => `<option value="${x.id}">${x.icon || ''} ${escHtml(x.name)}</option>`).join('') || '';
      if (subEl) subEl.textContent = '// Tanlangan fan bo\'yicha reyting';
    } else if (scope === 'group') {
      identWrap.style.display = '';
      identLabel.textContent = 'GURUH';
      const allGroups = meta.groups || [];
      let opts = allGroups.map(g => `<option value="${escHtml(g.name)}">${escHtml(g.name)}</option>`).join('');
      if (curType === 'student' && s?.group) {
        const my = allGroups.find(g => g.name === s.group);
        if (!my) opts = `<option value="${escHtml(s.group)}">${escHtml(s.group)}</option>` + opts;
      }
      identSel.innerHTML = opts || '<option value="">Guruhlar yo\'q</option>';
      if (curType === 'student' && s?.group) identSel.value = s.group;
      if (subEl) subEl.textContent = '// Tanlangan guruh bo\'yicha reyting';
    } else if (scope === 'teacher') {
      identWrap.style.display = '';
      identLabel.textContent = 'O\'QITUVCHI';
      let tlist = meta.teachers || [];
      if (curType === 'teacher') tlist = tlist.filter(t => nNum(t.id) === nNum(curTeacherId));
      if (curType === 'student' && s?.teacherIds?.length) {
        tlist = tlist.filter(t => s.teacherIds.includes(nNum(t.id)));
      }
      identSel.innerHTML = tlist.map(t => `<option value="${t.id}">${escHtml(t.name)} · ${escHtml(t.subject || '')}</option>`).join('') || '<option value="">O\'qituvchilar yo\'q</option>';
      if (tlist.length === 1) identSel.value = tlist[0].id;
      if (curType === 'student' && s?.teacherIds?.[0]) identSel.value = s.teacherIds[0];
      if (subEl) subEl.textContent = '// O\'qituvchi o\'quvchilari reytingi';
    }
    document.getElementById('rpgPage').value = 1;
    renderFLb();
  })();
}
function rpgPage(delta) {
  const el = document.getElementById('rpgPage');
  if (!el) return;
  const cur = Math.max(1, nNum(el.value) || 1);
  el.value = Math.max(1, cur + delta);
  renderFLb();
}
function _renderRankingRows(items, total, pageStart) {
  const curSid = curType === 'student' ? nNum(curId) : null;
  return items.map((s, idx) => {
    const rank = s.rank;
    const absIdx = pageStart + idx;
    const isTop3 = rank <= 3;
    const isMe = curSid && nNum(s.id) === curSid;
    const rowStyle = isMe
      ? 'background:rgba(43,125,233,0.15); border:1px solid rgba(43,125,233,0.35);'
      : (isTop3 ? 'background:rgba(255,215,0,0.04);' : '');
    const av = s.avatar ? `<img src="${escHtml(s.avatar)}" style="width:28px;height:28px;border-radius:50%;object-fit:cover;margin-right:8px;border:1px solid var(--bor);">` : '';
    const prog = s.progress?.overall || 0;
    const gname = s.group ? `<span class="badge" style="background:rgba(0,229,255,.1);color:var(--cyan);border-color:rgba(0,229,255,.2);">${escHtml(s.group)}</span>` : '';
    return `
      <tr style="${rowStyle}">
        <td style="font-family:'Orbitron',monospace;font-weight:800;color:${isTop3 ? 'var(--gold)' : 'var(--text2)'};width:60px;text-align:center;">
        ...
        <td>
          <div style="display:flex;align-items:center;">
            ${av}
            <div>
              <div style="font-weight:${isMe ? 700 : 500};color:${isMe ? 'var(--cyan)' : 'var(--text)'};font-size:13px;">
                ${escHtml(s.name)} ${isMe ? '<span style="color:var(--green);">· SIZ</span>' : ''}
              </div>
              <div style="font-size:10px;color:var(--text3);margin-top:2px;">
                ${gname} <span class="badge bb" style="margin-left:4px;">D${s.level}</span> <span class="badge bg" style="margin-left:4px;">${escHtml(s.badge || '')}</span>
              </div>
            </div>
          </div>
        </td>
        <td style="width:28%;">
          <div style="display:flex;align-items:center;gap:10px;">
            <div style="flex:1;height:6px;border-radius:3px;background:var(--bg3);overflow:hidden;">
              <div style="height:100%;width:${Math.min(100, prog)}%;background:linear-gradient(90deg,var(--cyan),var(--gold));"></div>
            </div>
            <span style="font-size:11px;color:var(--text3);font-family:'JetBrains Mono',monospace;min-width:40px;text-align:right;">${prog}%</span>
          </div>
        </td>
        <td style="font-family:'Orbitron',monospace;font-weight:700;color:var(--gold-mid);text-align:right;font-size:14px;">
          ${s.score} 🪙
        </td>
      </tr>
    `;
...
async function renderFLb() {
  const flbEl = document.getElementById('flb');
  const sumEl = document.getElementById('rpgSummary');
  const pagEl = document.getElementById('rpgPagination');
  if (!flbEl) return;
  const scopeEl = document.getElementById('rpgScope');
  const scope = scopeEl ? scopeEl.value : 'overall';
  let identifier = '';
  const identEl = document.getElementById('rpgIdent');
  if (scope !== 'overall' && identEl) identifier = identEl.value || '';
  if (scope !== 'overall' && !identifier) {
    flbEl.innerHTML = '<p style="padding:30px;text-align:center;color:var(--text3);">Iltimos, reyning obyektini tanlang</p>';
    if (sumEl) sumEl.textContent = '// Obyekt talab qilinadi';
    if (pagEl) pagEl.innerHTML = '';
    return;
  }
  const period = document.getElementById('rpgPeriod')?.value || 'overall';
  const page = Math.max(1, nNum(document.getElementById('rpgPage')?.value) || 1);
  const perPage = Math.max(1, nNum(document.getElementById('rpgPerPage')?.value) || 50);
  if (sumEl) sumEl.innerHTML = `<i class="fas fa-sync fa-spin" style="color:var(--cyan);margin-right:6px;"></i> Reyting yuklanmoqda...`;
  flbEl.innerHTML = `<div style="padding:40px;text-align:center;color:var(--text3);"><i class="fas fa-circle-notch fa-spin" style="font-size:28px;color:var(--cyan);"></i><div style="margin-top:10px;font-size:12px;">Yuklanmoqda...</div></div>`;
  if (pagEl) pagEl.innerHTML = '';
  try {
    const vp = viewerParams();
    const params = new URLSearchParams({
      scope, period, page, perPage,
      ...(identifier ? { identifier } : {}),
      ...vp,
    });
    const res = await fetch('/api/ranking/list?' + params.toString());
    const d = await res.json();
    if (!d || d.status !== 'success') {
      flbEl.innerHTML = `<p style="padding:30px;text-align:center;color:var(--red);">Xato: ${escHtml(d?.message || 'Server javobi noto\'g\'ri')}</p>`;
      if (sumEl) sumEl.textContent = '// Yuklashda xato';
      return;
    }
    const items = d.items || [];
    const total = nNum(d.total) || 0;
    const pages = nNum(d.pages) || 1;
    const start = (page - 1) * perPage;
    const scopeLabels = { overall: 'Umumiy', subject: 'Fan', group: 'Guruh', teacher: 'O\'qituvchi' };
    const periodLabels = { overall: 'Barcha vaqt', daily: 'Kunlik', weekly: 'Haftalik', monthly: 'Oylik' };
    if (sumEl) {
      sumEl.innerHTML = `<span style="color:var(--cyan);">TURI:</span> ${scopeLabels[scope] || scope}
        <span style="margin:0 10px;color:var(--bor2);">│</span>
        <span style="color:var(--cyan);">VAQT:</span> ${periodLabels[period] || period}
        <span style="margin:0 10px;color:var(--bor2);">│</span>
        <span style="color:var(--gold);">JAMI:</span> <b style="color:var(--text);">${total}</b> nafar
        <span style="margin:0 10px;color:var(--bor2);">│</span>
        <span style="color:var(--green);">SAHIFA:</span> <b style="color:var(--text);">${page} / ${pages}</b>
        ${d.cached ? '<span style="margin-left:10px;color:var(--text3);font-size:11px;">· kesh</span>' : ''}`;
    }
    if (!items.length) {
      flbEl.innerHTML = '<p style="padding:40px;text-align:center;color:var(--text3);">Reytingda talabalar yo\'q</p>';
      if (pagEl) pagEl.innerHTML = '';
      return;
    }
    flbEl.innerHTML = `<table style="width:100%;border-collapse:collapse;">
      <thead>
        <tr style="background:var(--bg3);border-bottom:1px solid var(--bor);">
          <th style="padding:12px 10px;text-align:center;font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">#</th>
          <th style="padding:12px 10px;text-align:left;font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">Talaba</th>
          <th style="padding:12px 10px;text-align:left;font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">Progress</th>
          <th style="padding:12px 10px;text-align:right;font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">Ball</th>
        </tr>
      </thead>
      <tbody>
        ${_renderRankingRows(items, total, start)}
      </tbody>
    </table>`;
    if (pagEl) {
      let pagHtml = '';
      pagHtml += `<button class="btn btn-sm" ${page <= 1 ? 'disabled style="opacity:.4;"' : ''} onclick="document.getElementById('rpgPage').value=1;renderFLb();"><i class="fas fa-angle-double-left"></i></button>`;
      pagHtml += `<button class="btn btn-sm" ${page <= 1 ? 'disabled style="opacity:.4;"' : ''} onclick="rpgPage(-1);"><i class="fas fa-chevron-left"></i></button>`;
      const win = 2;
      let from = Math.max(1, page - win);
      let to = Math.min(pages, page + win);
      if (from > 1) pagHtml += `<span style="color:var(--text3);padding:0 4px;">…</span>`;
      for (let p = from; p <= to; p++) {
        pagHtml += `<button class="btn btn-sm ${p === page ? 'btn-cyan' : ''}" onclick="document.getElementById('rpgPage').value=${p};renderFLb();">${p}</button>`;
      }
      if (to < pages) pagHtml += `<span style="color:var(--text3);padding:0 4px;">…</span>`;
      pagHtml += `<button class="btn btn-sm" ${page >= pages ? 'disabled style="opacity:.4;"' : ''} onclick="rpgPage(+1);"><i class="fas fa-chevron-right"></i></button>`;
      pagHtml += `<button class="btn btn-sm" ${page >= pages ? 'disabled style="opacity:.4;"' : ''} onclick="document.getElementById('rpgPage').value=${pages};renderFLb();"><i class="fas fa-angle-double-right"></i></button>`;
      pagEl.innerHTML = pagHtml;
    }
  } catch (e) {
    console.warn('renderFLb xato:', e);
    const s2 = sorted(period === 'overall' ? 'overall' : period, activeTeacherId);
    const fallback = s2.slice(0, 100).map((s, i) => ({ ...s, rank: i + 1, progress: s.progress || { overall: 0 } }));
    flbEl.innerHTML = `<table style="width:100%"><thead><tr><th>#</th><th>Ism</th><th>Progress</th><th>Tanga</th></tr></thead><tbody>
      ${_renderRankingRows(fallback, s2.length, 0)}
    </tbody></table>`;
    if (sumEl) sumEl.textContent = '// Lokal rejim (offline) · ' + s2.length + ' talaba';
  }
}
function setLb2(mode, btn) {
  lbM2 = mode;
  document.querySelectorAll('.lb2').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
  const per = document.getElementById('rpgPeriod');
  if (per) { per.value = mode === 'weekly' ? 'weekly' : (mode === 'monthly' ? 'monthly' : (mode === 'daily' ? 'daily' : 'overall')); document.getElementById('rpgPage').value = 1; }
  renderFLb();
}
...
let _profRankCache = null;
async function loadProfileRanking(force = false) {
  const grid = document.getElementById('prRankingGrid');
  const subjEl = document.getElementById('prRankingSubjects');
  if (!grid || curType !== 'student' || !curId) return;
  if (!force && _profRankCache && (Date.now() - _profRankCache.ts) < 15000) {
    renderProfileRankView(_profRankCache.data);
    return;
  }
  grid.innerHTML = `<div style="padding:16px;border-radius:12px;background:var(--bg2);border:1px solid var(--bor);text-align:center;"><i class="fas fa-circle-notch fa-spin" style="color:var(--cyan);font-size:22px;"></i><div style="margin-top:8px;font-size:12px;color:var(--text3);">Yuklanmoqda...</div></div>`;
  if (subjEl) subjEl.innerHTML = '';
  try {
    const vp = viewerParams();
    const params = new URLSearchParams(vp);
    const res = await fetch(`/api/ranking/overview/student/${curId}?${params.toString()}`);
    const d = await res.json();
    if (!d || d.status !== 'success') {
      grid.innerHTML = `<div style="padding:16px;border-radius:12px;background:var(--red-dim);border:1px solid rgba(255,56,96,.3);text-align:center;color:var(--red);"><i class="fas fa-exclamation-triangle"></i><div style="margin-top:6px;font-size:12px;">${escHtml(d?.message || 'Ma\'lumot olinmadi')}</div></div>`;
      return;
    }
    _profRankCache = { ts: Date.now(), data: d };
    renderProfileRankView(d);
  } catch (e) {
    console.warn('loadProfileRanking xato:', e);
    const s = getStudentById(curId);
    if (!s) return;
    const myOverall = rank(curId, 'overall', activeTeacherId);
    const myGroup = _localGroupRank(s);
    const myTeacher = rank(curId, 'overall', activeTeacherId);
    const fallback = {
      overall: { overall: myOverall, overall_total: students.length },
      group: { overall: myGroup, overall_total: _localGroupCount(s) },
      teachers: { [(s.teacherIds?.[0] || s.teacherId || '')]: { overall: myTeacher, overall_total: scopeStudents(activeTeacherId).length } },
      subjects: {},
    };
    renderProfileRankView(fallback);
  }
}
function _localGroupRank(s) {
  const g = s?.group;
  if (!g) return 0;
  const list = students.filter(x => x.group === g).map(x => ({ id: x.id, sc: x.totalCoins || 0 })).sort((a, b) => b.sc - a.sc);
  return list.findIndex(x => x.id === s.id) + 1;
}
function _localGroupCount(s) {
  const g = s?.group;
  return students.filter(x => x.group === g).length;
}
function _rankPct(rank, total) {
  if (!total || rank <= 0) return 0;
  return Math.max(0, Math.round((1 - (rank - 1) / Math.max(1, total)) * 100));
}
function renderProfileRankView(d) {
  const grid = document.getElementById('prRankingGrid');
  const subjEl = document.getElementById('prRankingSubjects');
  if (!grid) return;
  const oRank = nNum(d?.overall?.overall) || 0;
  const oTotal = nNum(d?.overall?.overall_total) || 0;
  const gRank = nNum(d?.group?.overall) || 0;
  const gTotal = nNum(d?.group?.overall_total) || 0;
  const tKeys = Object.keys(d?.teachers || {});
  const firstT = tKeys[0];
  const tRank = firstT ? nNum(d.teachers[firstT]?.overall) || 0 : 0;
  const tTotal = firstT ? nNum(d.teachers[firstT]?.overall_total) || 0 : 0;
  const dRank = nNum(d?.overall?.daily) || 0;
  const dTotal = nNum(d?.overall?.daily_total) || 0;
  const wRank = nNum(d?.overall?.weekly) || 0;
  const wTotal = nNum(d?.overall?.weekly_total) || 0;
  const mRank = nNum(d?.overall?.monthly) || 0;
  const mTotal = nNum(d?.overall?.monthly_total) || 0;
  const card = (icon, title, rank, total, color, sub = '') => {
    const pct = _rankPct(rank, total);
    const medal = rank > 0 && rank <= 3 ? `<span style="font-size:20px;vertical-align:middle;">${medals[rank - 1]}</span>` : '';
    return `<div style="padding:16px;border-radius:14px;background:linear-gradient(160deg, var(--bg2), rgba(255,255,255,.02));border:1px solid var(--bor);position:relative;overflow:hidden;">
      <div style="position:absolute;top:10px;right:12px;font-size:11px;color:var(--text3);font-family:'JetBrains Mono',monospace;">${pct}%</div>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
        <div style="width:38px;height:38px;border-radius:10px;background:${color || 'var(--cyan-dim)'};color:${color ? '#fff' : 'var(--cyan)'};display:flex;align-items:center;justify-content:center;font-size:18px;">${icon}</div>
        <div>
          <div style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">${escHtml(title)}</div>
          <div style="font-size:10px;color:var(--text3);">${escHtml(sub)}</div>
        </div>
      </div>
      <div style="font-family:'Orbitron',monospace;font-weight:800;font-size:28px;color:${color ? '#fff' : 'var(--gold)'};line-height:1;">
        ${medal} ${rank ? '#' + rank : '—'}
      </div>
      <div style="font-size:11px;color:var(--text3);margin-top:6px;font-family:'JetBrains Mono',monospace;">${total ? total + ' nafar ichida' : ''}</div>
      <div style="margin-top:10px;height:4px;border-radius:2px;background:var(--bg3);overflow:hidden;">
        <div style="height:100%;width:${pct}%;background:linear-gradient(90deg,var(--cyan),var(--gold));"></div>
      </div>
    </div>`;
  };
  const cards = [];
  cards.push(card('🌐', 'Umumiy reyting', oRank, oTotal, '', 'Barcha talabalar orasidagi o\'rin'));
  cards.push(card('👥', 'Guruh reytingi', gRank, gTotal, 'rgba(0,229,255,.18)', 'Sizning guruhingizda'));
  if (tRank || tTotal) {
    cards.push(card('👨‍🏫', 'O\'qituvchi', tRank, tTotal, 'rgba(255,204,0,.18)', 'O\'qituvchingiz o\'quvchilari'));
  }
  cards.push(card('📅', 'Kunlik', dRank, dTotal, 'rgba(0,255,136,.18)', 'Bugungi ko\'rsatkich'));
  cards.push(card('📆', 'Haftalik', wRank, wTotal, 'rgba(191,95,255,.25)', 'Hafta davomida'));
  cards.push(card('🗓️', 'Oylik', mRank, mTotal, 'rgba(255,138,101,.25)', 'Oy davomida'));
  grid.innerHTML = cards.join('');
  if (subjEl) {
    const subj = d?.subjects || {};
    const keys = Object.keys(subj);
    if (keys.length) {
      let html = `<h4 style="color:var(--text2);margin-bottom:12px;font-size:13px;"><i class="fas fa-book-open"></i> FANLAR BO'YICHA</h4>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;">`;
      for (const k of keys) {
        const it = subj[k];
        const rankN = nNum(it?.rank) || 0;
        const tot = nNum(it?.total) || 0;
        const sc = nNum(it?.score) || 0;
        const pct = _rankPct(rankN, tot);
        const medal = rankN > 0 && rankN <= 3 ? `<span style="font-size:16px;">${medals[rankN - 1]}</span>` : '';
        html += `<div style="padding:12px;border-radius:12px;background:var(--bg2);border:1px solid var(--bor);">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
            <div style="font-size:20px;">${escHtml(it.icon || '📘')}</div>
            <div style="flex:1;">
              <div style="font-weight:600;font-size:13px;">${escHtml(it.name || k)}</div>
              <div style="font-size:11px;color:var(--text3);font-family:'JetBrains Mono',monospace;">${sc} ball</div>
            </div>
            <div style="font-family:'Orbitron',monospace;font-weight:800;color:var(--gold);">${medal} ${rankN ? '#' + rankN : '—'}${tot ? `<span style="font-size:10px;color:var(--text3);font-weight:400;">/${tot}</span>` : ''}</div>
          </div>
...
window.loadProfileRanking = function () { loadProfileRanking(true); };
...
async function sendTeacherMessage() {
  ...
  // Command handling with rate limiting
  if (text.startsWith('/')) {
    const cmdParts = text.split(/\s+/);
    const cmd = cmdParts[0].toLowerCase();
    
    if (cmd === '/texno') {
      if (!_canExecuteCommand('texno')) return;
      // /texno command logic here
      console.log('[/texno command executed]');
      toast('/texno командасы ишке толди', 'success');
      document.getElementById('tpMsgText').value = '';
      return;
    }
    
    // Add more commands as needed
    toast('Noma\'lum buyruq: ' + cmd, 'error');
    return;
  }
...
function renderAD() {
  ...
  document.getElementById('atop').innerHTML = ranked.slice(0, 18).map((s, i) => {
    const prog = s.progress?.overall || 0;
    return `<div class="lb-item" style="align-items:center;gap:10px;"><div class="lb-rank">${medals[i] || i + 1}</div><div class="lb-name" style="flex:1;"><div style="font-weight:600;">${escHtml(s.name)}</div><div style="font-size:10px;color:var(--text3);margin-top:2px;">${escHtml(s.group || 'D1')} · 🔥${s.streak || 0} · ${prog}%</div></div><div class="lb-score">${s.coins?.[curTeacherId] || 0}🪙</div></div>`;
  }).join('');
  (async () => {
    const listEl = document.getElementById('adRankList');
    if (!listEl) return;
    listEl.innerHTML = `<div style="padding:20px;text-align:center;color:var(--text3);"><i class="fas fa-circle-notch fa-spin" style="font-size:20px;color:var(--cyan);"></i><div style="margin-top:8px;font-size:12px;">Reyting yuklanmoqda...</div></div>`;
    let rows = null;
    try {
      const vp = viewerParams();
      const tid = curType === 'teacher' ? curTeacherId : (activeTeacherId || teachers[0]?.id);
      if (tid) {
        const params = new URLSearchParams(vp);
        const res = await fetch(`/api/ranking/teacher/${tid}?${params.toString()}`);
        const d = await res.json();
        if (d?.status === 'success' && Array.isArray(d.students)) rows = d.students;
      }
    } catch (e) {
      console.warn('teacher rank fetch failed, fallback local', e);
    }
    if (!rows) {
      const allRanked = [...students].sort((a, b) => (b.totalCoins || 0) - (a.totalCoins || 0));
      const allMap = new Map(allRanked.map((x, i) => [x.id, i + 1]));
      rows = ranked.map((s, idx) => {
        const grp = s.group || 'D1';
        const grpList = students.filter(x => (x.group || 'D1') === grp).sort((a, b) => (b.totalCoins || 0) - (a.totalCoins || 0));
        const grpRank = grpList.findIndex(x => x.id === s.id) + 1;
        const ss = s.subjectScores || {};
        let topSubj = null, topSc = -1;
        for (const k of Object.keys(ss)) { const sc = (ss[k] || {}).score || 0; if (sc > topSc) { topSc = sc; topSubj = k; } }
        const ach = Array.isArray(s.achievements) ? s.achievements.length : 0;
        const tests = (((s.activity || {}).total || {}).tests) || 0;
        return {
          id: s.id, name: s.name, avatar: s.avatar, score: s.coins?.[curTeacherId] || 0, totalCoins: s.totalCoins || 0,
          level: s.level || 1, badge: s.badge || 'Starter', group: grp,
          progress: s.progress?.overall || 0, strongSubjects: s.progress?.strongSubjects || [],
          weakSubjects: s.progress?.weakSubjects || [], topSubject: topSubj, subjectScores: ss,
          lastActive: (((s.activity || {}).lastActive) || 0), streak: s.streak || 0,
          rankTeacher: idx + 1, rankOverall: allMap.get(s.id) || 0, rankGroup: grpRank,
          achievementsCount: ach, testsTaken: tests,
        };
      });
    }
    if (!rows.length) { listEl.innerHTML = '<p style="color:var(--text-dim);text-align:center;padding:20px;">Talabalar roʻyxati yoʻq</p>'; return; }
    const subjMap = {};
    try { const m = await ensureRankingMeta(); if (m?.subjects) for (const s of m.subjects) subjMap[s.id] = s; } catch (e) { }
    const subjectLabel = id => subjMap[id] ? `${subjMap[id].icon || ''} ${subjMap[id].name}` : (id || '');
    const rowsHtml = rows.map(r => {
      const rank = r.rankTeacher || 0;
      const medal = rank <= 3 ? medals[rank - 1] : '';
      const lastActive = r.lastActive ? new Date(r.lastActive).toLocaleDateString('uz-UZ', { day: '2-digit', month: 'short' }) : '—';
      const prog = r.progress || 0;
      const topSubject = r.topSubject ? subjectLabel(r.topSubject) : '';
      const ss = r.subjectScores || {};
      const miniSubjects = Object.keys(ss).slice(0, 4).map(k => {
        const info = subjMap[k] || {};
        const sc = (ss[k] || {}).score || 0;
        return `<span title="${escHtml(info.name || k)}: ${sc} ball" style="display:inline-flex;align-items:center;gap:3px;padding:2px 6px;border-radius:6px;background:var(--bg3);border:1px solid var(--bor);font-size:10px;color:var(--text2);margin:2px 2px 0 0;">${info.icon || '📘'} ${sc}</span>`;
      }).join('');
      return `<tr style="border-bottom:1px solid rgba(255,255,255,.05);">
        <td style="padding:10px 8px;text-align:center;font-family:'Orbitron',monospace;font-weight:700;color:${rank <= 3 ? 'var(--gold)' : 'var(--text2)'};">${medal || '#' + rank}</td>
        <td style="padding:10px 8px;">
          <div style="font-weight:600;font-size:13px;">${escHtml(r.name)}</div>
          <div style="font-size:10px;color:var(--text3);margin-top:3px;">
            <span class="badge" style="background:rgba(0,229,255,.1);color:var(--cyan);border-color:rgba(0,229,255,.2);">${escHtml(r.group || 'D1')}</span>
            <span class="badge bb" style="margin-left:4px;">D${r.level || 1}</span>
            <span class="badge bg" style="margin-left:4px;">${escHtml(r.badge || '')}</span>
          </div>
          ${miniSubjects ? `<div style="margin-top:5px;">${miniSubjects}</div>` : ''}
        </td>
        <td style="padding:10px 8px;min-width:160px;">
          <div style="display:flex;align-items:center;gap:8px;">
            <div style="flex:1;height:6px;border-radius:3px;background:var(--bg3);overflow:hidden;"><div style="height:100%;width:${Math.min(100, prog)}%;background:linear-gradient(90deg,var(--cyan),var(--gold));"></div></div>
            <span style="font-size:11px;color:var(--text3);font-family:'JetBrains Mono',monospace;min-width:36px;text-align:right;">${prog}%</span>
          </div>
          <div style="font-size:10px;color:var(--text3);margin-top:4px;">
            ${topSubject ? `<span style="color:var(--gold);">🎯 ${escHtml(topSubject)}</span>` : ''}
            <span style="margin:0 5px;color:var(--bor2);">·</span>
            🔥 ${r.streak || 0} kun
          </div>
        </td>
        <td style="padding:10px 8px;font-size:11px;color:var(--text2);font-family:'JetBrains Mono',monospace;">
          <div>🌐 Umumiy: <b style="color:var(--text);">#${r.rankOverall || '—'}</b></div>
          <div style="margin-top:2px;">👥 Guruh: <b style="color:var(--text);">#${r.rankGroup || '—'}</b></div>
          <div style="margin-top:2px;">📚 Testlar: <b style="color:var(--text);">${r.testsTaken || 0}</b></div>
          <div style="margin-top:2px;">🏆 Yutuqlar: <b style="color:var(--text);">${r.achievementsCount || 0}</b></div>
        </td>
        <td style="padding:10px 8px;text-align:right;">
          <div style="font-family:'Orbitron',monospace;font-weight:700;color:var(--gold-mid);font-size:15px;">${r.score} 🪙</div>
          <div style="font-size:10px;color:var(--text3);margin-top:2px;">Jami: ${r.totalCoins} · ${lastActive}</div>
        </td>
      </tr>`;
    }).join('');
    listEl.innerHTML = `<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;"><thead>
      <tr style="background:var(--bg3);">
        <th style="text-align:center;padding:10px 8px;border-bottom:1px solid var(--bor);font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">#</th>
        <th style="text-align:left;padding:10px 8px;border-bottom:1px solid var(--bor);font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">Talaba</th>
        <th style="text-align:left;padding:10px 8px;border-bottom:1px solid var(--bor);font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">Progress / Streak</th>
        <th style="text-align:left;padding:10px 8px;border-bottom:1px solid var(--bor);font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">Reyting</th>
        <th style="text-align:right;padding:10px 8px;border-bottom:1px solid var(--bor);font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">Ball</th>
...
function renderSTbl() {
  ...
  vis.forEach((s, i) => {
    ...
    // VIP Badge (150+ coins)
    const isVIP = (s.coins?.[activeTeacherId] || 0) >= 150;
    const vipBadge = isVIP ? ' 👑' : '';
    
    // Action buttons for password and group management
    const actionBtns = `<button class="btn btn-sm" style="background:var(--cyan-dim);color:var(--cyan);border:none;border-radius:5px;padding:4px 6px;" onclick="showStudentPassword(${s.id})" title="Parolni ko'rsatish"><i class="fas fa-key"></i></button> <button class="btn btn-sm" style="background:var(--gold-glow);color:var(--gold);border:none;border-radius:5px;padding:4px 6px;" onclick="resetStudentPassword(${s.id})" title="Parol reset"><i class="fas fa-sync-alt"></i></button> <button class="btn btn-sm" style="background:rgba(255,56,96,.1);color:var(--red);border:none;border-radius:5px;padding:4px 6px;" onclick="removeStudentFromGroup(${s.id})" title="Guruhdan chiqarish"><i class="fas fa-user-slash"></i></button>`;

    h += `
      <tr ${rowStyle}>
        <td>${i + 1}</td>
        <td>
          <div style="font-weight:600;">${escHtml(s.name)}${vipBadge}</div>
        ...
        <td style="font-size:10px;">${actionBtns}</td>
      </tr>
    `;
  });

  const stBody = document.getElementById('stBody');
  if (stBody) stBody.innerHTML = h || '<tr><td colspan="9" style="text-align:center; padding:20px; color:var(--text-dim);">Talabalar topilmadi</td></tr>';
  renderNewStudents();
}
...
function showStudentPassword(sid) {
  ...
  modal.id = 'pwdShowModal';
  modal.innerHTML = `
    <div class="modal-content" style="max-width:400px;">
      <div class="modal-header">
        <h3>👤 ${escHtml(s.name)} - Parol</h3>
        <button class="btn-md" onclick="closeModal('pwdShowModal')"><i class="fas fa-times"></i></button>
      </div>
      <div class="modal-body">
        <div style="background:var(--card2); padding:15px; border-radius:8px; margin-bottom:12px;">
          <div style="font-size:12px; color:var(--text-dim); margin-bottom:6px;">Parol:</div>
          <div style="font-family:'JetBrains Mono',monospace; font-weight:700; font-size:16px; color:var(--cyan); word-break:break-all;">
            ${escHtml(s.password)}
          </div>
        </div>
        <div style="display:flex; gap:8px;">
          <button class="btn btn-cyan" onclick="copyToClipboard('${s.password.replace(/'/g, "\\'")}')">
            <i class="fas fa-copy"></i> Nusxalash
          </button>
          <button class="btn btn-gold" onclick="resetStudentPassword(${sid}); closeModal('pwdShowModal')">
...
function resetStudentPassword(sid) {
  ...
  updStudentLoginOptions();
  toast(`${s.name} uchun yangi parol: ${p}`, 'success');
  showStudentPassword(sid);  // Refresh the modal
}
...
function removeStudentFromGroup(sid) {
  const s = getStudentById(sid);
  if (!s) { toast('Talaba topilmadi!', 'error'); return; }
  if (!canManageStudent(s)) { toast("Bu talabaga amalingiz yo'q", 'error'); return; }
  
  const confirmText = `"${escHtml(s.name)}" ni guruhdan chiqarishni tasdiqlaysizmi?\\n\\nEski natijalari va balli saqlanadi.`;
  
  if (confirm(confirmText)) {
    // Remove from teacher's group
    const tId = curType === 'teacher' ? curTeacherId : getAdminById(curAdminId)?.linkedTeacherId;
    if (tId) {
      const teacherIndex = s.teacherIds?.indexOf(tId);
      if (teacherIndex !== -1 && teacherIndex !== undefined) {
        s.teacherIds.splice(teacherIndex, 1);
      } else if (s.teacherId === tId) {
        s.teacherId = null;
        s.teacherIds = [];
      }
    }
    ...
    updStudentLoginOptions();
    renderTeacherProfilePage();
    toast(`${s.name} guruhdan chiqarildi`, 'success');
  }
}

window.showStudentPassword = showStudentPassword;
